...
const LINUX_DESKTOP_NAME = "dev.codax.launcher.desktop";
...
function linuxExecutable(app) {
  const stableLauncher = process.env.CODAX_LAUNCHER_EXECUTABLE?.trim();
  if (stableLauncher && path.isAbsolute(stableLauncher)) return stableLauncher;
  const appImage = process.env.CODAX_APPIMAGE?.trim() || process.env.APPIMAGE?.trim();
