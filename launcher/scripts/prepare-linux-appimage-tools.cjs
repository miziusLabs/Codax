...
async function main() {
  if (process.platform !== "linux" || process.arch !== "x64") {
    throw new Error("Codax AppImage tool preparation requires Linux x64");
  }
  const source = process.env.CODAX_LINUX_LIBNOTIFY?.trim();
  if (!source || !path.isAbsolute(source) || !fs.statSync(source, { throwIfNoEntry: false })?.isFile()) {
    throw new Error(
      "AppImage tool preparation requires CODAX_LINUX_LIBNOTIFY from scripts/prepare-linux-libnotify.sh",
    );
  }
  ...
  const outputRoot = path.resolve(
    process.env.CODAX_APPIMAGE_TOOLS_OUTPUT
