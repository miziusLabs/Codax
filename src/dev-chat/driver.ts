...
export const DEV_CHAT_SYSTEM_INSTRUCTIONS = [
  "You are running inside the Codax DEV outer-harness simulator.",
...
function requestBody(
  ...
  return {
    ...
    metadata: { codax_dev: true, chat_name: state.name },
  };
}
...
export class DevChatDriver {
  ...
  async send(
    ...
    for (let round = 0; round < 64; round += 1) {
      ...
      const response = await responseRequest(new Request("http://codax.dev/v1/responses", {
        method: "POST",
  ...
  private async compactInput(
    ...
    const response = await compactRequest(new Request("http://codax.dev/v1/responses/compact", {
      method: "POST",
      headers: {
